...
def run_folder_backup(folder_id, folder_data):
    try:
        ...
        if token and chat_id:
            ...
            try: 
                loop.run_until_complete(upload_pipeline(files_to_upload, token, chat_id, folder_id, should_auto_delete, use_zip, fingerprints_to_save))
            finally: 
                loop.close()
...
class FolderConfigWindow(ctk.CTkToplevel):
    def __init__(self, parent, folder_id=None, folder_data=None):
        ...
        self.past_tk, self.past_ch = get_past_credentials()
        ...
        # Core Credentials Setup
        ctk.CTkLabel(main_scroll, text="Telegram Bot Token API Key Reference:", anchor="w").pack(fill="x", padx=20, pady=(5,0))
        self.token_entry = ctk.CTkComboBox(main_scroll, values=self.past_tk)
        self.token_entry.set(self.folder_data.get("token", ""))
        self.token_entry.pack(fill="x", padx=20, pady=4)
        
        ctk.CTkLabel(main_scroll, text="Telegram Private target Chat ID String:", anchor="w").pack(fill="x", padx=20, pady=(2,0))
        self.chat_entry = ctk.CTkComboBox(main_scroll, values=self.past_ch)
        self.chat_entry.set(self.folder_data.get("chat_id", ""))
        self.chat_entry.pack(fill="x", padx=20, pady=4)
